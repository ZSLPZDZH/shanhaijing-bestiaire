...
本次把旧的按记录数描述异兽数量的口径调整为“109 条图鉴记录、89 个异兽名称”，属于事实校正：`beasts_data.json` 有 109 条记录，但其中包含同名条目，去重后为 89 个名称。
...
2. 一张来自真实生产页面的预览图；
3. 核心能力：109 条图鉴记录、89 个异兽名称、八类经文、搜索筛选、详情阅读；
4. 数据字段说明与适合的使用场景；
5. 技术实现与目录结构；
6. 最短本地运行步骤；
7. 内容来源、准确性声明和版权边界。
...
预览图只承担“让用户快速判断页面风格和内容”的作用。截图会话中临时隐藏展示旧数量口径的 `.hero-stats` 和 `.results-count`，必要时仅临时收紧 `.hero` 的高度；这些调整不写回生产文件，也不改变线上站点。图片应压缩到适合 GitHub 加载的体积，同时保持标题、筛选栏和首屏卡片可辨认。
...
- README 中的数量与 `beasts_data.json` 一致：记录数为 109、唯一名称数为 89、分类数为 8；
